...
- 性质：规划文档；Phase A–E 主体已在 `feat/v2.3-architecture-debt` 实施

## 实施进度（2026-09-10）

| 阶段 | 状态 | 说明 |
|---|---|---|
| A 契约置换 / pin 几何 | 完成 | `main/domains/pin/geometry.js` + 盘点文档 |
| B1–B6 域拆分 | 完成 | pin / capture / long-capture / record / recognition / search / settings-effects |
| C AI 边界 | 完成 | `main/services/ai` 单入口 + client 实现迁入 + `resolveProtocolAdapter` |
| D1 tokens | 完成 | `shared/tokens.css` |
| D2 config 拆分 | 部分 | 纯 model helpers 已抽出；整页 render 拆分待后续 |
| D3 内联样式 | 完成 | pin/action 外部 CSS |
| E 门禁 | 完成 | `npm run check:architecture` |

`main.js`：3583 → **2022** 行。测试基线：484+。
