    ...
    for (const [kind, channels] of [
      ...
      for (const channel of channels) {
        ...
        if (existing && existing.kind !== kind) {
          throw new Error(`IPC channel kind conflict: ${channel}`)
        }
...
function createSecureIpcMain({
  ...
  function deny(channel, reason, details = {}) {
    try {
      onBlocked({ channel, reason, ...details })
    } catch {}
  ...
  function authorize(channel, event) {
    ...
    if (!senderFrame || senderFrame !== sender.mainFrame) {
      return deny(channel, 'non-top-level-frame')
    }
    ...
    try {
      ownerAllowed = authorizeRole(allowedPage.role, win, event) === true
    } catch {}
    if (!ownerAllowed) {
      return deny(channel, 'window-owner-mismatch', { role: allowedPage.role })
    }

    return {
      allowed: true,
      role: allowedPage.role,
      win
    }
  }
