...
// Abandon selection extraction (UIA/Accessible/Clipboard) if the target app
// (notably Chromium) blocks longer than this. Prevents wedging both the hook
// host and the target UI thread.
constexpr DWORD SELECTION_EXTRACT_TIMEOUT_MS = 750;
...
class SelectionHook : public Napi::ObjectWrap<SelectionHook>
{
  ...
  private:
    ...
    bool GetSelectedTextTimed(HWND hwnd, TextSelectionInfo &selectionInfo);
    bool GetSelectedTextBody(HWND hwnd, TextSelectionInfo &selectionInfo, IUIAutomation *uia, uint64_t epoch);
    bool IsSelectionEpochCurrent(uint64_t epoch) const;
    bool GetTextViaUIAutomation(HWND hwnd, TextSelectionInfo &selectionInfo, IUIAutomation *uia, uint64_t epoch);
    bool TryGetTextViaTextPattern(IUIAutomationElement *pElement, TextSelectionInfo &selectionInfo,
                                  bool allowDocumentFallback);
    bool GetTextViaAccessible(HWND hwnd, TextSelectionInfo &selectionInfo, uint64_t epoch);
    ...
    // Bumped on timeout so abandoned workers discard late results.
...
Napi::Value SelectionHook::GetCurrentSelection(const Napi::CallbackInfo &info)
{
    ...
    try
    {
        ...
        if (!GetSelectedTextTimed(hwnd, selectionInfo) || IsTrimmedEmpty(selectionInfo.text))
        {
...
void SelectionHook::ProcessMouseEvent(Napi::Env env, Napi::Function function, MouseEventContext *pMouseEvent)
{
    ...
    if (shouldDetectSelection)
    {
        ...
        if (hwnd)
        {
            if (currentInstance->GetSelectedTextTimed(hwnd, selectionInfo) && !IsTrimmedEmpty(selectionInfo.text))
            {
...
bool SelectionHook::GetSelectedText(HWND hwnd, TextSelectionInfo &selectionInfo)
{
    ...
    const uint64_t epoch = selection_epoch.load();
    const bool ok = GetSelectedTextBody(hwnd, selectionInfo, pUIAutomation, epoch);
    is_processing.store(false);
    return ok;
}

bool SelectionHook::IsSelectionEpochCurrent(uint64_t epoch) const
{
    return selection_epoch.load() == epoch;
}

/**
 * Run selection extraction on a worker thread with its own COM/UIA apartment.
 * If the target app blocks past SELECTION_EXTRACT_TIMEOUT_MS, abandon the worker
 * (detach) and return false so input hooks stay responsive.
 */
bool SelectionHook::GetSelectedTextTimed(HWND hwnd, TextSelectionInfo &selectionInfo)
{
    if (!hwnd)
        return false;
    ...
    // Claim a fresh epoch; any prior abandoned worker becomes stale.
    const uint64_t epoch = selection_epoch.fetch_add(1) + 1;

    struct TimedTask
    {
        SelectionHook *hook = nullptr;
        HWND hwnd = nullptr;
        TextSelectionInfo info;
        std::atomic<bool> done{false};
        // 0=running, 1=main abandoned, 2=worker completed
    ...
    std::thread worker([task]()
                       {
        const bool comOk = SUCCEEDED(CoInitializeEx(NULL, COINIT_APARTMENTTHREADED | COINIT_DISABLE_OLE1DDE));
        ...
        SelectionHook *hook = task->hook;
        bool success = false;
        if (hook->IsSelectionEpochCurrent(task->epoch))
        {
            TextSelectionInfo local;
            success = hook->GetSelectedTextBody(task->hwnd, local, uia, task->epoch);
            if (success && hook->IsSelectionEpochCurrent(task->epoch))
            {
                task->info = std::move(local);
            }
            else
            {
                success = false;
        ...
        int expected = 0;
        if (task->ownership.compare_exchange_strong(expected, 2))
        {
            task->done.store(true);
        }
        else
        {
            // Main already abandoned this attempt; free the task here.
            delete task;
        } });
    ...
    {
        if (GetTickCount() - startedAt >= SELECTION_EXTRACT_TIMEOUT_MS)
        {
            int expected = 0;
            if (task->ownership.compare_exchange_strong(expected, 1))
            {
                // Abandon: invalidate epoch so a late worker cannot publish UIA side effects.
                selection_epoch.fetch_add(1);
                is_processing.store(false);
                worker.detach();
                return false;
            }
            // Worker finished in the race window; fall through to join.
            break;
        }
        Sleep(1);
    }
    ...
    selectionInfo = std::move(task->info);
    delete task;
    return true;
}

bool SelectionHook::GetSelectedTextBody(HWND hwnd, TextSelectionInfo &selectionInfo, IUIAutomation *uia, uint64_t epoch)
{
    if (!hwnd)
        return false;
    ...
    if (!IsSelectionEpochCurrent(epoch))
        return false;

    // First try UI Automation (supported by modern applications)
    if (uia && GetTextViaUIAutomation(hwnd, selectionInfo, uia, epoch))
    {
        if (!IsSelectionEpochCurrent(epoch))
            return false;
    ...
    if (GetTextViaAccessible(hwnd, selectionInfo, epoch))
    {
        if (!IsSelectionEpochCurrent(epoch))
            return false;
 ...
 */
bool SelectionHook::GetTextViaUIAutomation(HWND hwnd, TextSelectionInfo &selectionInfo, IUIAutomation *uia,
                                           uint64_t epoch)
{
    if (!uia || !hwnd)
        return false;
    ...
    HRESULT hr = uia->ElementFromHandle(hwnd, &pElement);
    ...
    // Get the focused element - using local scope to ensure proper cleanup
    {
        ...
        hr = uia->GetFocusedElement(&pFocusedElement);
        ...
        if (!result && IsSelectionEpochCurrent(epoch))
        {
            IUIAutomationTreeWalker *pTreeWalker = nullptr;
            hr = uia->get_ControlViewWalker(&pTreeWalker);
 ...
 */
bool SelectionHook::GetTextViaAccessible(HWND hwnd, TextSelectionInfo &selectionInfo, uint64_t epoch)
{
    if (!hwnd)
        return false;
    if (!IsSelectionEpochCurrent(epoch))
