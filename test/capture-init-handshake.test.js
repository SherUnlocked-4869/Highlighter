...
test('capture initialization waits for the renderer-ready handshake', () => {
  assert.match(captureDomain, /captureWindow\._captureRendererReady = false/)
  assert.match(captureDomain, /function sendCaptureInit\(win\)[\s\S]*!win\._captureRendererReady[\s\S]*win\.webContents\.send\('capture:init', win\._captureInit\)/)
  assert.match(captureDomain, /ready: \(event\) => \{[\s\S]*win\._captureRendererReady = true[\s\S]*sendCaptureInit\(win\)/)
  assert.doesNotMatch(main, /captureWindow\.setFullScreen\(true\)/)
  assert.doesNotMatch(captureDomain, /captureWindow\.setFullScreen\(true\)/)
