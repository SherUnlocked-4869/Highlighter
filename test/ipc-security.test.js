...
test('IPC policy covers all 94 main-process registration points', () => {
  ...
  assert.deepEqual(
    policies.get('toolbar:action').pages.map(({ role }) => role),
    ['main', 'toolbar']
  )
})

test('every renderer-to-main preload channel has the matching page policy', () => {
  ...
  for (const [preload, role] of preloadRoles) {
    ...
    const calls = source.matchAll(/ipcRenderer\.(invoke|send)\('([^']+)'/g)
    for (const [, method, channel] of calls) {
      ...
      assert.ok(
        policy.pages.some((page) => page.role === role),
        `${preload} cannot call its declared channel: ${channel}`
      )
    }
  }
})

test('secure IPC accepts registered top-level owners and rejects cross-page senders', () => {
  const root = path.resolve(__dirname, '..')
  ...
  assert.equal(
    ipcMain.handlers.get('shell:open-external')(eventFor(recognitionWindow), 'https://example.com'),
    'shell:open-external'
  )
  assert.throws(
    () => ipcMain.handlers.get('settings:get')(eventFor(recognitionWindow)),
    /IPC sender not authorized/
  )
  assert.throws(
    () => ipcMain.handlers.get('capture:copy')(eventFor(mainWindow), {}),
    /IPC sender not authorized/
  )
...
test('secure IPC rejects unregistered, navigated, iframe, destroyed, and stale-owner senders', () => {
  const root = path.resolve(__dirname, '..')
  const ipcMain = createIpcMain()
  const blocked = []
  const calls = []
  ...
  assert.throws(
    () => ipcMain.handlers.get('settings:get')(eventFor(unregistered)),
    /IPC sender not authorized/
  )
  ...
  assert.throws(
    () => ipcMain.handlers.get('settings:get')(eventFor(navigated)),
    /IPC sender not authorized/
  )

  const iframe = createWindow(root, 'config/config.html', 'main')
  const childFrame = { url: iframe.webContents.mainFrame.url }
  assert.throws(
    () => ipcMain.handlers.get('settings:get')(eventFor(iframe, childFrame)),
    /IPC sender not authorized/
  )
  ...
  assert.throws(
    () => ipcMain.handlers.get('settings:get')(eventFor(destroyed)),
    /IPC sender not authorized/
  )

  const staleOwner = createWindow(root, 'capture/capture.html', 'stale-capture')
  assert.throws(
    () => ipcMain.handlers.get('capture:copy')(eventFor(staleOwner), {}),
    /IPC sender not authorized/
  )
