...
test('release workflows enforce version, native, signing, integrity, and draft gates', () => {
  ...
  assert.match(ci, /HIGHLIGHTER_REQUIRE_CAPTURE_RUNTIME/)
  assert.match(ci, /npm run verify:package/)
  assert.match(release, /environment: release/)
  assert.match(release, /HIGHLIGHTER_REQUIRE_CAPTURE_RUNTIME/)
  assert.match(release, /npm run audit:dependencies/)
  assert.match(release, /npm run test:e2e/)
  assert.match(release, /npm run test:coverage/)
...
test('release verifier launches the packaged executable before accepting artifacts', () => {
  ...
  assert.match(verifier, /verify-packaged-startup\.ps1'\) -ExecutablePath \$portable\.FullName/)
