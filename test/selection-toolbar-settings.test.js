...
test('main process normalizes toolbar settings and routes configured custom prompts', () => {
  ...
  assert.match(main, /createTranslateStream\(apiKey, text, action\.prompt\)/)
  assert.match(main, /createExplainStream\(apiKey, text, action\.prompt\)/)
  assert.match(main, /createCustomStream\(apiKey, text, action\.prompt\)/)
