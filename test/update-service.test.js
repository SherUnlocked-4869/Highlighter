...
test('normalizes update failures and removes secrets from logs and notes', async () => {
  ...
  assert.equal(normalizeUpdateError(new Error('ERR_UPDATER_INVALID_UPDATE_INFO YAMLException')).code, 'update-metadata-invalid')
  assert.equal(normalizeUpdateError(new Error('ENOTFOUND')).code, 'network-error')
  assert.equal(normalizeUpdateError(new Error('ERR_UPDATER_CHANNEL_FILE_NOT_FOUND HttpError: 404 keep-alive: timeout=5')).code, 'network-error')
  assert.equal(normalizeUpdateError(new Error('net::ERR_CONTENT_LENGTH_MISMATCH')).code, 'network-error')
