...
const DEFAULT_TOOLBAR_THINKING = Object.freeze({ translate: 'off', explain: 'high' })
const CUSTOM_ACTION_DEFAULT_THINKING = 'high'
...
function normalizeCustomActions(value) {
  ...
  for (const item of value) {
    ...
    ids.add(id)
    const action = { id, name, prompt, enabled: item.enabled !== false }
    if (THINKING_LEVELS.includes(item.thinking)) action.thinking = item.thinking
    actions.push(action)
  }
  return actions
}
...
function normalizeSelectionToolbar(value = {}) {
  ...
  for (const action of Object.keys(OPTIONAL_TOOLBAR_ACTIONS)) {
    if (config.buttons && Object.hasOwn(config.buttons, action)) buttons[action] = config.buttons[action] !== false
  }
  ...
  const validActions = new Set([
    ...
    ...Object.keys(OPTIONAL_TOOLBAR_ACTIONS),
  ...
  for (const action of [...TOOLBAR_ACTION_ORDER, ...customActions.map((item) => customActionKey(item.id))]) {
...
function getVisibleToolbarActions(config = DEFAULT_SELECTION_TOOLBAR) {
  ...
  return normalized.order.filter((action) => {
    ...
    if (OPTIONAL_TOOLBAR_ACTIONS[action]) return true
...
function getToolbarActionDefinition(config, action) {
  const normalized = normalizeSelectionToolbar(config)
  const builtin = BUILTIN_TOOLBAR_ACTIONS[action] || OPTIONAL_TOOLBAR_ACTIONS[action]
  if (builtin) {
    return {
      ...builtin,
...
function buildOpenUrl(text) {
  const value = String(text || '').trim()
  if (!value) return ''
  if (/^https?:\/\//i.test(value)) return value
  return `http://${encodeURI(value)}`
}

function normalizeThinkingLevel(value, fallback = 'off') {
  return THINKING_LEVELS.includes(value) ? value : fallback
}

function normalizeToolbarThinking(value = {}) {
  const config = value && typeof value === 'object' && !Array.isArray(value) ? value : {}
  return {
    translate: normalizeThinkingLevel(config.translate, DEFAULT_TOOLBAR_THINKING.translate),
    explain: normalizeThinkingLevel(config.explain, DEFAULT_TOOLBAR_THINKING.explain)
  }
}

function getToolbarActionThinking(toolbarConfig, thinkingConfig, action) {
  if (AI_TOOLBAR_ACTIONS.has(action)) {
    return normalizeToolbarThinking(thinkingConfig)[action] || 'off'
  }
  const customId = parseCustomActionKey(action)
  if (!customId) return 'off'
  const normalized = normalizeSelectionToolbar(toolbarConfig)
  const custom = normalized.customActions.find((item) => item.id === customId)
  return normalizeThinkingLevel(custom?.thinking, CUSTOM_ACTION_DEFAULT_THINKING)
}
